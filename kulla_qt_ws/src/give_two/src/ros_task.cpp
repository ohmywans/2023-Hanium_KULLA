...
namespace give_two{
    ...
    void ros_task::init_nh()
      {
        ...
        sub_give_mode =nh->subscribe("/give_two", 10, sub_give_take_callback);

        // give_mode에서 Servo 1 제어 : unlock 1 버튼
        pub_unlock1 = nh->advertise<std_msgs::Int32>("/servo2", 10);
        // give_mode에서 Servo 1 제어 : lock 1 버튼
        pub_lock1 = nh->advertise<std_msgs::Int32>("/servo2", 10);

        // Publish : 사용자 1, P1 = 1, P2 = 2, P3 = 3, P4 = 4
        pub_p1 = nh->advertise<std_msgs::Int32>("/website_second_topic", 10); // 사용자 1의 거점 publish할 때, 토픽명
        pub_p2 = nh->advertise<std_msgs::Int32>("/website_second_topic", 10);
        pub_p3 = nh->advertise<std_msgs::Int32>("/website_second_topic", 10);
        pub_p4 = nh->advertise<std_msgs::Int32>("/website_second_topic", 10);
    ...
    void ros_task::run()
    {
      ...
      while(ros::ok())
      {
        ...
        {
          Q_EMIT give_take_ui();
          cout<<"From give_take to give_mode"<<endl;
          give_take_close = 0;
        }
